...
public class KycService {
    ...
    public KycSubmitResponse submitAndVerify(KycSubmitRequest req) {
        ...
        // recent_location nằm ở mặt TRƯỚC trong tài liệu, nếu back không có thì fallback từ front
        ...
        // API trả: face_swapping (boolean), fake_liveness (boolean), không có fake_liveness_prob
        boolean fakeLiveness = false;
        boolean faceSwapping = false;
        if (lvObj != null) {
            Object fl = lvObj.get("fake_liveness");
            Object fs = lvObj.get("face_swapping");
            if (fl instanceof Boolean) fakeLiveness = (Boolean) fl;
            if (fs instanceof Boolean) faceSwapping = (Boolean) fs;
        }
        ...
        // Theo tài liệu VNPT: response có object.prob (float) và object.msg ("MATCH"/"NOMATCH")
        ...
        try { if (birthDay != null && birthDay.contains("/")) dob = LocalDate.parse(birthDay, f); } catch (Exception ignored) {}
        try { if (issueDateS != null && issueDateS.contains("/")) issueDate = LocalDate.parse(issueDateS, f); } catch (Exception ignored) {}
